    ...
    $.fn.extend({
        ...
        diyUpload: function (opt, serverCallBack) {
            ...
            webUploader.on('uploadSuccess', function (file, response) {
                if (response.code == 1) {
                    var html = '<input type="hidden" value="' + response.msg + '" name="' + response.id + '" code="success"/>';
                    ...
                    var html = '<input type="hidden" value="" name="' + response.id + '" code="fail"/>';
                    $(document.body).append(html);
                    var $fileBox = $('#fileBox_' + file.id);
                    var $diyBar = $fileBox.find('.diyBar');
                    showDiyProgress(0, $diyBar, '上传失败!' + response.msg);
                    var err = '上传失败! 文件:' + file.name;
                    if (errorCallBack) {
                        errorCallBack(err);
                    }
                }
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        if ($parentFileBox.find('.diyButton').length <= 0) {
            ...
            var div = '<div class="diyButton"> \
						<a class="diyStart btn btn-min btn-red" href="javascript:void(0)">开始上传</a> \
						<a class="diyCancelAll mlt10 btn btn-min btn-white" href="javascript:void(0)">全部取消</a> \
					</div>';
...
function uploadFile(url) {
    ...
    var html = '<div id="uploadBox" style="display: none;padding:10px; width:720px;"> <div id="uploadDiv"></div></div>';
    $(document.body).append(html);
    $('#uploadDiv').diyUpload({
        url: url,
        success: function (data) {
            if (data.code == 2 && data.loginUrl != '') {
