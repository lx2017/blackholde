...
function FormRenderFn(e, t) {
    ...
    function() {
        ...
        o.formData = e[o.dataType](o.formData) || !1
    } (),
    i.fieldRender = function(e) {
        var t = "",
        r = "",
        a = "",
        i = e.label || "",
        ...
        c = e.values || [];
        e.id = e.name,
        e.type = e.subtype || e.type,
        e.required && (e.required = null, e["aria-required"] = "true", s = '<span class="required">*</span>'),
        "hidden" !== e.type && (l && (l = '<span class="tooltip-element" tooltip="' + l + '">?</span>'), r = '<label for="' + e.id + '">' + i + " " + s + " " + l + "</label>");
        var f = e.label;
        delete e.label,
        delete e.description;
        var u = n.attrString(e);
        switch (e.type) {
        case "textarea":
        case "rich-text":
            delete e.type;
            var d = e.value || "";
            t = r + "<textarea " + u + ">" + d + "</textarea>";
            break;
        case "select":
            var p;
            if (e.type = e.type.replace("-group", ""), c) for (var v = 0; v < c.length; v++) c[v].selected || delete c[v].selected,
            p = n.attrString(c[v]),
            a += "<option " + p + ">" + c[v].label + "</option>";
            t = r + "<select " + u + ">" + a + "</select>";
            break;
        case "checkbox-group":
        case "radio-group":
            var m = void 0;
            if (e.type = e.type.replace("-group", ""), c) {
                for (var b = "checkbox" === e.type ? e.name + "[]": e.name, y = void 0, h = 0; h < c.length; h++) m = Object.assign({},
                e, c[h]),
                m.selected && (delete m.selected, m.checked = null),
                m.name = b,
                m.id = e.id + "-" + h,
                y = n.attrString(m),
                a += "<input " + y + ' /> <label for="' + m.id + '">' + m.label + "</label><br>";
                if (e.enableOther || e["enable-other"]) {
                    var g = {
                        id: e.id + "-other",
                        className: (e["class"] || e.className) + " other-option"
                    };
                    y = n.attrString(Object.assign({},
                    e, g)),
                    a += "<input " + y + ' /> <label for="' + g.id + '">' + o.label.other + '</label> <input type="text" data-other-id="' + g.id + '" name="' + b + '" id="' + g.id + '-value" style="display:none;" />'
                }
            }
            t = r + '<div class="' + e.type + '-group">' + a + "</div>";
            break;
        case "text":
        case "password":
        case "email":
        ...
        case "autocomplete":
            ...
            break;
        case "color":
            t = r + " <input " + u + "> " + o.label.selectColor;
            break;
        case "button":
        case "submit":
            t = "<button " + u + ">" + f + "</button>";
        ...
        if ("hidden" !== e.type) {
            var k = e.id ? "form-group field-" + e.id: "";
            t = n.markup("div", t, {
                className: k
            })
        } else t = n.markup("input", null, e);
        return t
    },
    ...
    c = [];
    if (o.formData) {
        for (var f = 0; f < o.formData.length; f++) c.push(i.fieldRender(o.formData[f]));
        if (o.render) {
            if (o.container) {
                var u = n.markup("div", c, {
                    className: "rendered-form"
                });
                o.container = o.container instanceof jQuery ? o.container[0] : o.container,
                o.container.emptyContainer(),
                o.container.appendChild(u)
            } else t && (t.emptyContainer(), t.appendFormFields(c));
            s(),
            o.notify.success(o.label.formRendered)
        } else r.markup = c.map(function(e) {
            return e.innerHTML
        }).join("")
    } else {
        var d = n.markup("div", o.label.noFormData, {
            className: "no-form-data"
        });
        c.push(d),
        o.notify.error(o.label.noFormData)
    }
    return r
}
...
function(e) {
    var t = function(t, n) {
        ...
        var o = '<div class="kct-on">' + a.labels.on + "</div>",
...
fbUtils.trimObj = function(e) {
    var t = [null, void 0, "", !1];
    for (var n in e) fbUtils.inArray(e[n], t) && delete e[n];
    return e
},
...
fbUtils.markup = function(e) {
    ...
    o = {
        ...
        array: function(e) {
            for (var t = 0; t < e.length; t++) r = i(e[t]),
            o[r](e[t])
        }
    };
...
fbUtils.parseAttrs = function(e) {
    var t = e.attributes,
    n = {};
    for (var r in t) t.hasOwnProperty(r) && (n[t[r].name] = t[r].value);
    return n
},
fbUtils.parseOptions = function(e) {
    ...
    n = {},
    r = [];
    if (t.length) for (var a = 0; a < t.length; a++) n = fbUtils.parseAttrs(t[a]),
    n.label = t[a].textContent,
    r.push(n);
    return r
},
fbUtils.parseXML = function(e) {
    ...
    r = [];
    if (n) for (var a = n.getElementsByTagName("field"), i = 0; i < a.length; i++) {
        var o = fbUtils.parseAttrs(a[i]);
        o.values = fbUtils.parseOptions(a[i]),
        r.push(o)
    }
    return r
},
