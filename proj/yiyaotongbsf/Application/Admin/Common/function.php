 ...
 * 清空指定路径(文件或文件夹)
 * @param $path
 * @param $flag bool 是否删除当前目录,默认不
 */
function dropDir($path,$flag=false){
    if (is_file($path)) {
        unlink($path);
    } else if (is_dir($path)) {
...
/**
 * 删除上传文件
 * @param int $type 0-清空目录,1删除上传文件
 * @param $path array 路径数组
 * @return bool
 */
function deleteFile($type=1,$path=array()){
    if($type==0){
        dropDir('.'.UPLOADIFY_TEMP);//清空临时目录
    }else{
        //文件路径
