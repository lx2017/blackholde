...
class AuthGroupModel extends Model
{
    ...
    protected $_validate = array(
        array('title', 'require', '必须设置组名', Model::MUST_VALIDATE, 'regex', Model::MODEL_INSERT),
        array('title', '1,20', '组名最多20字符', Model::MUST_VALIDATE, 'length', Model::MODEL_INSERT),
     ...
     * @return mixed
     */
    public function deleteByGroupIds(array $groupIds)
    {
        M()->startTrans();
        $flag1 = $this->where(array('id' => array('in', $groupIds)))->delete();
        $authGroupAccessModel = new AuthGroupAccessModel();
        $flag2 = $authGroupAccessModel->where(array('group_id' => array('in', $groupIds)));
        $authGroupModuleModel = new AuthGroupModuleModel();
        $flag3 = $authGroupModuleModel->where(array('group_id' => array('in', $groupIds)));
     ...
     * @return mixed
     */
    public function getAllByField($field)
    {
        return $this->field($field)->select();
    }

    /**
     * 根据groupid查出组的title和描述信息
     * @param $groupId
     * @return mixed
     */
    public function getGroupById($groupId)
    {
        return $this->field('id,title,description')->where(array('id' => $groupId))->find();
    }
}
