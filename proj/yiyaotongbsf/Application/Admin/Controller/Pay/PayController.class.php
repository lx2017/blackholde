<?php
namespace Admin\Controller\Pay;
...
class PayController extends AdminController{
	...
	public function __construct(){
		parent::__construct();
		//$this->model=new \Admin\Model\User\UserModel();
		
	}
	/*
	 * 一、生成密钥公钥参照文档，需在服务器安装openssl，并通过openssl命令行生成。
	 * 二、各种支付模式需调整下面index方法里的channel
	 * */
	public function index(){
		require_once(APP_PATH."Admin/Library/Ping/init.php");

// api_key 获取方式：登录 [Dashboard](https://dashboard.pingxx.com)->点击管理平台右上角公司名称->开发信息-> Secret Key
$api_key = 'sk_test_4Sm9i5LqbnL4rbjbTGvb18G0';
// app_id 获取方式：登录 [Dashboard](https://dashboard.pingxx.com)->点击你创建的应用->应用首页->应用 ID(App ID)
$app_id = 'app_5arPK8PyrHi18i5C';

// 此处为 Content-Type 是 application/json 时获取 POST 参数的示例
$input_data = json_decode(file_get_contents('php://input'), true);
//var_dump($input_data);die;
if (empty($input_data['channel']. 'alipay_wap') || empty($input_data['amount'].'10')) {
    echo 'channel or amount is empty';
    exit();
}
echo "<pre>";
$input_data['channel']='wx_pub';
$channel = strtolower($input_data['channel']);
$amount=1;
//$amount = $input_data['amount']?$input_data['amount']:10;
...
// 设置私钥内容方式1
\Pingpp\Pingpp::setPrivateKeyPath(APP_PATH.'Admin/Library/Ping/example/rsa_private_key.pem');
