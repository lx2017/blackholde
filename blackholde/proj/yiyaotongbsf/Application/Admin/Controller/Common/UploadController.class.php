...
class UploadController extends Controller
{
    ...
    /**
     * 处理excel文件上传
     * @param $handle1 string 处理模型目录
     * @param $handle2 string 处理模型控制器
     * @param $method string 处理方法
     */
    public function excel($handle1,$handle2,$method){
        if(!$handle1 || !$handle2 || !$method) $this->ajaxReturn(array('code'=>2,'msg'=>'处理失败:未指定处理模型'));
        $config = C('UPLOADIFY_EXCEL_CONF');
        //设置上传
        $upload = new Upload($config);
        $rst = $upload->uploadOne($_FILES['Filedata']);
        if($rst!==false){
            //得到excel文件路径
            $file = $config['rootPath'].$rst['savepath'].$rst['savename'];
            //得到模型和方法
            $name = '\\'.($this->plat).'\\Model\\'.$handle1.'\\'.$handle2.'Model';
            $model = new $name();
            $method = $method?:$this->method;
            //处理结果
            $res = $model->$method($file);
            $this->ajaxReturn($res);
        }else{
            $this->ajaxReturn(array('code'=>1,'msg'=>'上传失败:'.$upload->getError()));
        }
